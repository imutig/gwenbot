...
/**
 * Bot Login Callback - Handles Twitch OAuth callback for bot account
 * Saves the bot token to the database
 */
export async function GET(request: NextRequest) {
    const searchParams = request.nextUrl.searchParams
    const code = searchParams.get('code')
    const state = searchParams.get('state')
    const savedState = request.cookies.get('bot_login_state')?.value
    ...
    try {
        ...
        const response = NextResponse.redirect(`${baseUrl}?success=bot_logged_in&user=${user.display_name}`)
        response.cookies.delete('bot_login_state')

        return response
    } catch (error) {
        console.error('Bot login callback error:', error)
